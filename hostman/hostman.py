...
import python_hosts.hosts as phosts
import python_hosts.utils as putils
import sys
import os
import datetime
import shutil
...
def add(entry_line=None, hosts_path=None, force_add=False):
    ...
    :return:
    """
    hosts_entry = phosts.HostsEntry.str_to_hostentry(entry_line)
    if not hosts_entry:
...
def import_from_file(hosts_path=None, file_path=None):
    ...
    if not os.path.exists(file_path):
        ...
        hosts = phosts.Hosts(path=hosts_path)
...
def remove(address_to_remove=None, names_to_remove=None, remove_from_path=None):
    ...
    """
    hosts = phosts.Hosts(path=remove_from_path)
    remove_result = None
    write_result = False
...
if __name__ == '__main__':
    ...
    if not putils.is_readable(path):
    ...
    if arguments.get('add'):
        final_message = None
        if not putils.is_writeable(path):
            output_message({'result': 'failed',
